...
@LastEditors: 华豪
@LastEditTime: 2019-12-17 15:48:16
'''
...
def hot_news():
    ...
    try:
        ...
        cur.execute("select * from index_index where abs_titles='hot_news'")
        rows = cur.fetchall()
        if rows:
            conn.commit()
        else:
    ...
    try:
        ...
        cur.execute("select * from index_index where abs_titles='hot_pane_news'")
        rows = cur.fetchall()
        if rows:
            conn.commit()
        else:
    ...
    try:
        ...
        cur.execute("select * from index_index where abs_titles='online_news'")
        rows = cur.fetchall()
        if rows:
            conn.commit()
        else:
